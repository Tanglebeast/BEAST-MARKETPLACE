...
import { getRpcUrl, getCurrentNetwork } from '../components/networkConfig'; // Stellen Sie sicher, dass der Pfad korrekt ist
import { nftCollections } from '../NFTCollections';
import CustomPopup from '../components/AlertPopup';
import { getGasEstimate, connectWallet, web3OnlyRead } from '../components/utils'; // Import der connectWallet-Funktion aus den utils
...
const PollDetails = () => {
  const [web3, setWeb3] = useState(web3OnlyRead); // Standardmäßig read-only Web3
        ...
        if (window.ethereum && window.ethereum.isMetaMask) {
          try {
            ...
            if (accounts.length === 0) {
              setErrorMessage('No wallet found. Please connect your wallet.');
            } else {
            ...
            setErrorMessage('Error initializing we3 or contract.');
          }
        } else {
          setErrorMessage('Please initialize Metamask!');
        }
      } else {
        try {
          // Verwenden der read-only Web3-Instanz aus den Utils
          const readOnlyContract = new web3OnlyRead.eth.Contract(NFT_VOTING_ABI, NFT_VOTING_ADDRESS);
          setContract(readOnlyContract);
          await fetchPoll(readOnlyContract, null, web3OnlyRead);
        } catch (error) {
  ...
  const fetchPoll = async (contractInstance, account = null, web3Instance) => {
    try {
      ...
      if (pollData) {
        ...
        if (
          location.pathname.includes('/fairvote') ||
          artistNameFromPoll.toLowerCase() === getArtistName().toLowerCase()
        ) {
          const endTime = parseInt(pollData.endTime, 10);
      ...
      console.error(`Error fetching poll with ID: ${pollIdInt}:`, error);
    }
  };
  ...
  const fetchUserVotes = async (contract, account) => {
    try {
      ...
      const votesMap = votesForPoll.reduce((acc, vote) => {
        acc[vote.optionId] = (acc[vote.optionId] || 0) + parseInt(vote.voteCount, 10);
        return acc;
      }, {});
  ...
  const getArtistName = () => {
    // Implementieren Sie eine Funktion, die den aktuellen Künstlernamen zurückgibt.
    // Dies könnte über Props, Context oder eine andere zentrale Zustandsverwaltung erfolgen.
    // Für dieses Beispiel nehmen wir an, dass `artistName` als Prop übergeben wird.
    // Wenn nicht, passen Sie diese Funktion entsprechend an.
    return ''; // Passen Sie dies entsprechend an
  };
  ...
  const handleVote = async () => {
    ...
    try {
      ...
      if (receipt.status) {
        setModalMessage('Votes placed successfully!');
        setShowModal(true);
      ...
      setModalMessage('Error placing vote. Do you own enough votingrights?');
      setShowModal(true);
    }
  };
  ...
  const handleRemoveVote = async () => {
    ...
    try {
      ...
      console.log('Transaction:', receipt);
      if (receipt.status) {
        setModalMessage('Votes removed successfully!');
        setShowModal(true);
        await fetchPoll(contract, connectedAccount, web3);
      } else {
        throw new Error('Transaction failed');
      ...
      setModalMessage('Error removing votes');
      setShowModal(true);
    }
  };
  ...
  const artistNameFromPoll = uniqueArtists.length === 1 ? uniqueArtists[0] : '';

  const getTotalVotes = () => {
    if (!poll) return 0;
    return poll.votes.reduce((sum, votes) => sum + parseInt(votes, 10), 0);
  };
    ...
    <div className="poll-details-container column">
      ...
      {poll.isERC20Poll ? (
        <div>
          ...
          <div className='flex center-ho'>
            <img src='/crown.png' className='h20px mr5' alt="Crown Icon" />
            <span className="erc20-voting-label s14 gold">BEAST TOKEN HOLDER</span>
          </div>
        </div>
      ) : (
        <div>
          ...
          <div className="nft-contract-addresses mb15 mt0">
            {Array.isArray(nftData) && nftData.map((nft, index) => (
              <div className='column flex VisibleLink fit-content' key={index}>
                <span className='grey'> {artistNameFromPoll}</span>
      ...
      <ul className="poll-options">
        {Array.isArray(poll.options) && poll.options.map((option, index) => {
          const votes = parseInt(poll.votes[index], 10);
      ...
      <div className='centered column'>
        <label className='mb15' htmlFor="voteCount">AMOUNT VOTES:</label>
        <input
      ...
      <div className='column centered w100'>
        {connectedAccount ? (
          <>
            <button onClick={handleVote} disabled={isPollEnded} className="vote-button mb10 mt15 w50 white">
              <h2 className='mt5 mb5 s16'>PLACE VOTES</h2>
            </button>
            <button onClick={handleRemoveVote} disabled={isPollEnded} className="remove-vote-button w50 white">
              <h2 className='mt5 mb5 s16'>REMOVE VOTES</h2>
            </button>
          </>
        ) : (
          <button onClick={() => connectWallet(setConnectedAccount)} className="vote-button mb10 mt15 w50 white">
