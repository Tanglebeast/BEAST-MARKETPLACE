// src/components/PollsList.jsx
import React, { useState, useEffect } from 'react';
import { Link, useLocation } from 'react-router-dom';
import Web3 from 'web3';
import '../styles/Pollist.css';
import { getRpcUrl } from '../components/networkConfig'; // Anpassung des Pfads, falls nötig
import { nftCollections } from '../NFTCollections';
import { artistList } from '../ArtistList';
import PollFilter from './PollFilter';
import SearchBar from '../components/SearchBar';
import LoadingSpinner from '../Assets/loading-spinner';
import { web3OnlyRead } from '../components/utils'; // Import der read-only Web3-Instanz
...
const PollsList = ({ artistName }) => {
  const [web3Instance, setWeb3] = useState(web3OnlyRead); // Standardmäßig read-only Web3
    ...
    const init = async () => {
      ...
      if (storedAccount) {
        if (window.ethereum && window.ethereum.isMetaMask) {
          // Nur MetaMask zulassen
          try {
            const web3 = new Web3(window.ethereum);
            setWeb3(web3);

            // Verwenden Sie die bereits verbundene Account aus localStorage
            const currentAccount = storedAccount;

            setConnectedAccount(currentAccount);
            const nftVotingContract = new web3.eth.Contract(NFT_VOTING_ABI, NFT_VOTING_ADDRESS);
          ...
          setErrorMessage('Bitte installieren Sie MetaMask!');
        }
      } else {
        try {
  ...
  const fetchPolls = async (contractInstance) => {
    ...
    for (let batch = 1; batch < batches; batch++) {
      ...
      setPolls((prevPolls) => {
        ...
        const uniquePolls = combinedPolls.filter(
    ...
    for (let i = start; i < start + limit; i++) {
      try {
        ...
        if (
          ...
          artistNameFromPoll.toLowerCase() === artistName.toLowerCase()
        ) {
          const endTime = parseInt(poll.endTime, 10);
  ...
  if (loading && loadedBatches === 0) {
    return (
      <div className="loading-container mt150 mb150">
        <LoadingSpinner filled={false} textColor="currentColor" size={100} className="loading-gif" />
      </div>
    );
  }
  ...
  return (
    <div className="polls-list-container">
      {/* Der "Connect Wallet"-Button wurde entfernt, da die Verbindung im Header gehandhabt wird */}
              ...
              <div className="w100 flex center-ho space-between mediacolumn2">
                <div className="w30 w100media">
                  ...
                  <SearchBar
                    ...
                    placeholder="Suche nach Künstler oder Umfrage"
                  />
                </div>
              ...
              {sortedPolls.length === 0 ? (
                ...
                getCurrentPolls().map((poll) => {
                  ...
                  const profileImage = artist
                    ...
                    : poll.isERC20Poll
                    ? '/images/tanglebeasts-profile.png'
                    : '/images/default-profile.png'; // Fallback-Bild
                  ...
                  return (
                    <div key={poll.id} className="poll-card">
                      ...
                      <div className="flex center-ho space-between">
                        ...
                        <img src={profileImage} alt={`${poll.artistName} Profil`} className="artist-profile-image-poll" />
                      </div>
